...
12 ターンのセッション（Qwen3.5-122B-A10B via LM Studio）での実測値:

```
 #    Dur |   Input  Create    Read    Out |  Hit%
-----------------------------------------------------------------
...
| 指標 | 値 |
| --- | --- |
| 全体キャッシュヒット率 | **46.7%**（初回コールドスタート含む） |
| 2 回目以降のヒット率 | **37〜50%**（会話が長くなるほど ~50% に収束） |

> ヒット率が約 50% になるのは、ターンごとに新しいメッセージが追加されるため。先頭のシステムプロンプト＋過去メッセージがキャッシュから読まれ、末尾の新規部分だけが通常処理されます。
