...
この挙動改善は **LM Studio の GGUF / llama.cpp バックエンド利用時を前提**にしています。**MLX バックエンド利用時は正常に機能しません**。
...
- LM Studio を使う場合は **GGUF / llama.cpp バックエンド** を利用すること

> 注意: LM Studio で **MLX バックエンド** を使用した場合、このプロキシの正規化による期待どおりの効果は得られず、正常動作しません。
